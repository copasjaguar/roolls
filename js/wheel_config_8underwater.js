// 8 sectors underwater

var wheel_config_8underwater = {
    ...
    createWheel: function(scene) {
        ...
        // add sprites
        scene.wheelborder = addSprite('wheelborder', 0, 147, depth);

        scene.wCont = scene.add.container(scene.centerX, scene.centerY + 182); 

        scene.wheel = addSprite("wheel", 0 - scene.wCont.x, 182 - scene.wCont.y, depth)?.setOrigin(0.5, 0.5).setAngle(22.5);
        scene.lightsector = addSprite('lightsector', 0, 182, depth)?.setAlpha(0)?.setOrigin(0.5, 0.96);

        // setup spin button
        scene.spinbutton = addSprite('spinbutton', 0, 660, depth); 
        ...
        scene.pointer = addSprite('pointer', 0, -85, depth).setOrigin(0.5, 0.35);
        scene.centerpin = addSprite('centerpin', 0, 200, depth);
        // scene.pointerBorder = addSprite('pointerborder', 0, -0, depth);

        // adding the text field
        scene.prizeText = scene.add.bitmapText(scene.centerX, scene.centerY + 540, 'gameFont_1', 'Rode a Roleta', 38, 1).setOrigin(0.5);
    ...
    createLamps: function(scene){
        var lCount = 9; var lOffset = 268; var dY = 183; 
        scene.lampsArray = []; 
